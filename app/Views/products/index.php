...
<?= $this->section('title') ?>Sản phẩm & Dịch vụ<?= $this->endSection() ?>

<?= $this->section('content') ?>
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-900 mb-6">Sản phẩm & Dịch vụ</h1>
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <?php foreach($categories as $category): ?>
                    <a href="<?= site_url('products/category/' . $category['id']) ?>" class="px-4 py-2 rounded-lg <?= strpos(current_url(), 'category/' . $category['id']) !== false ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' ?>">
                        <?= $category['name'] ?>
                    </a>
                <?php endforeach; ?>
            </div>
